...
const PUBLIC_PREFIXES = ["/f/", "/c/", "/p/", "/s/", "/line-link/"];  // 公開フォーム／コンテンツ公開URL／ページ公開URL／流入経路リダイレクタ／LINE連携フォーム
...
//   ⚠️ /auth/confirm は token_hash 方式（verifyOtp）の着地点。
//      管理者が代理で送るメールは PKCE（/auth/callback）では成立しないため、
//      別端末で開かれうるメールリンクはすべてこちらを使う。
const PUBLIC_EXACT    = ["/set-password", "/auth/trial", "/auth/callback", "/auth/confirm"];  // 招待受諾・パスワード再設定・体験版の即時ログイン・メールリンク着地
