...
//   ⚠️ 画面側に生の色クラス・色値を直書きしないこと（brand.md §7）。
//      ホームの配色はここが唯一の正本。
//   赤の濃淡＋無彩色だけで作る。緑は「視聴済／既読」＝SUCCESS_CONFIG の役割にだけ使う。
// ============================================================

/** ブロックの共通スタイル */
export const HOME_SECTION = "mb-6";
/** 棚・ランキングの見出し行 */
export const HOME_RAIL_HEAD = "flex items-baseline gap-2.5 mb-2.5";
export const HOME_RAIL_TITLE = "text-[15px] font-black text-gray-900 tracking-tight";
export const HOME_RAIL_NOTE = "text-[12px] text-gray-400 font-bold";
export const HOME_RAIL_MORE = "ml-auto text-[12px] text-red-700 font-bold hover:underline whitespace-nowrap";

/**
 * 種別ピル（動画／資料／記事）。
 *   ⚠️ ContentView の KIND_PILL は資料＝indigo・記事＝emerald の多色。
 *      ホームに持ち込むと brand.md §1「多色は使わず赤の濃淡で強弱」に反し、
 *      緑が「視聴済」と衝突するため、ホーム用は無彩色＋赤で別に持つ。
 */
export const HOME_KIND_PILL: Record<string, string> = {
  video: "bg-red-600 text-white",
  doc:   "bg-neutral-200 text-neutral-700",
  none:  "bg-neutral-100 text-neutral-600",
};

/** ヒーロー（今月これだけは）の背景。画像が無いときの既定 */
export const HOME_HERO_FALLBACK =
  "linear-gradient(180deg,rgba(10,10,12,.10) 0%,rgba(10,10,12,.62) 52%,rgba(10,10,12,.90) 100%),"
  + "linear-gradient(118deg,#3f1013 0%,#7f1620 42%,#c2161f 100%)";
/** ヒーローの高さ（PC / スマホ） */
export const HOME_HERO_H = "min-h-[200px] sm:min-h-[280px]";

/** 棚カードのサムネイル既定背景（種別ごと。ContentView の既定カバーと揃える） */
export const HOME_THUMB_FALLBACK: Record<string, string> = {
  video: "linear-gradient(135deg,#17171b,#3a0a0e)",
  doc:   "linear-gradient(135deg,#2b2b31,#111)",
  none:  "linear-gradient(135deg,#c7d2fe,#e0e7ff)",
};

/**
 * 危険色（削除・不可逆操作）。
 *   brand.md §1 は危険色を constants.ts に集約すると定めているが、
 *   これまで SUCCESS_CONFIG しか無く未整備だった。ホーム設定の「削除」で必要になるため、
 *   SUCCESS_CONFIG と対になる形でここに定義する。
 *   ⚠️ アクセント用の red-* を危険色に流用しないこと（brand.md §1-2）。
 */
export interface DangerStyle {
  bg: string; border: string; text: string; icon: string; solid: string;
  /** 枠線ボタン1式（Tailwind は実行時の文字列合成を拾えないので、完成形で持つ） */
  outlineBtn: string;
}
export const DANGER_CONFIG: DangerStyle = {
  bg:     "bg-rose-50",
  border: "border-rose-200",
  text:   "text-rose-700",
  icon:   "text-rose-600",
  solid:  "bg-rose-600 text-white",
  outlineBtn: "border border-rose-200 text-rose-700 bg-white hover:bg-rose-50",
};
