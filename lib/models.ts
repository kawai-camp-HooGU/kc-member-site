...
export interface HomeBlockConfig {
  ...
  ctaLabel?: string;
  imageUrl?: string;
  /** hero：画像の左上に出す小見出し。未設定は「トピック」 */
  kicker?: string;
  /**
   * hero：背景画像のどこを見せるか（object-position 相当。0〜100の%）。
   *   横 focusX：0＝左端 / 50＝中央（既定） / 100＝右端
   *   縦 focusY：0＝上端 / 50＝中央（既定） / 100＝下端
   *   ⚠️ 高さ固定＋cover で切り抜くため、人物の顔が切れるかどうかはここで決まる。
   *      運営が編集画面のプレビューを見ながら合わせる。
   */
  focusX?: number;
  focusY?: number;
