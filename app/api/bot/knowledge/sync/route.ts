...
//   ・運営(requireOps)のみ。
//   ・body: { source: 'note'|'x'|'chat_bookmark'|'content'|'news', mode?: 'full'|'dry_run' }
...
const SOURCES: SourceType[] = ["note", "x", "chat_bookmark", "content", "news"];
...
export async function POST(request: Request) {
  try {
    ...
    if (!source || !SOURCES.includes(source)) {
      throw new HttpError(400, `source は ${SOURCES.join(" / ")} のいずれかを指定してください。`);
    }
