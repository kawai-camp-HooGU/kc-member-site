...
//   ⚠️ 高さ固定＋cover なので、画像のどこが見えるかは backgroundPosition で決まる。
//      既定は中央だが、人物の顔が切れることがあるため運営が
//      config.focusX / focusY（0〜100%）で調整できる（ホーム設定のプレビューで合わせる）。
// ============================================================
...
export function HeroBlock({
  ...
  const kicker = b.config.kicker || "トピック";
  // 画像の見せたい位置（既定は中央）
      ...
      >
        <div className="relative z-10 max-w-[620px]">
          <span className="inline-block rounded bg-red-600 text-white text-[11px] font-black
                           tracking-widest px-2.5 py-1 mb-2.5">{kicker}</span>
