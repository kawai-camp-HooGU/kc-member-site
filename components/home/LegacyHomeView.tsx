"use client";
// ============================================================
// 旧ホーム（改修前の会員ポータル・ホーム）— REQ-094 のフォールバック
//
//   ⚠️ これは REQ-094 以前の HomeView.tsx をそのまま移設したものです。
//      ホームのブロックが1つも表示されない状態（マイグレーション未適用／
//      運営がまだ組んでいない／全部非公開）のとき、新しいホームの代わりに
//      これを描いて「改修前とまったく同じ画面」を出します。
//
//   ⚠️ ロジックを変えないでください。
//      「設定が無いときは旧画面のまま」を保証するための退避なので、
//      ここに手を入れると保証が崩れます。手を入れたくなったら、
//      それは新しいブロック側（components/home/*）でやるべき変更です。
//
//   デザイン：案H「大タイル・ランチャー型」
...
// ── 本体 ──────────────────────────────────────────────────
export function LegacyHomeView({ onOpen, chatUnread = 0 }: Props) {
